...
test("SGPS architecture model is canonical, typed, and graph-valid", () => {
  ...
  for (const requiredKind of REQUIRED_PROJECT_KINDS) {
    assert.ok(
      ...
      `missing applicable kind ${requiredKind}`,
    );
  }
  ...
  for (const relation of model.relationships) {
    ...
    assert.ok(
      entitiesById.has(relation.from),
      `unknown from endpoint ${relation.from}`,
    );
    assert.ok(
      entitiesById.has(relation.to),
      `unknown to endpoint ${relation.to}`,
    );
    assert.notEqual(
      relation.from,
      relation.to,
      `self relationship ${relation.id}`,
    );
  }
});

test("SGPS marketing system binds architecture claims to immutable Git evidence", () => {
  const system = model.entities.find(
    (entity) => entity.id === "system.sgps-marketing",
  );
  ...
  assert.equal(
    system.sourceEvidence?.repository,
...
test("static-site model does not fabricate backend architecture", () => {
  ...
  const fabricated = model.entities.filter((entity) =>
    prohibitedKinds.has(entity.kind),
  );
  assert.deepEqual(
    fabricated,
    [],
...
test("derived architecture views are deterministic and explicitly non-authoritative", () => {
  execFileSync(
    process.execPath,
    ["scripts/generate-architecture-views.mjs", "--check"],
    {
      stdio: "pipe",
    },
  );
  const views = JSON.parse(
    readFileSync("architecture/derived-views.json", "utf8"),
  );
