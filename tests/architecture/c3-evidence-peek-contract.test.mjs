...
test("Evidence Peek uses canonical capability proof", () => {
  ...
  assert.match(
    peek,
    /import type \{ ProductProofLink \} from ["']@\/lib\/product-proof["']/,
  );
  ...
  for (const source of [peek, proof]) {
    assert.doesNotMatch(
      source,
      /\b(fetch|sendBeacon|XMLHttpRequest)\s*\(|localStorage|sessionStorage|<script\b/,
    );
    assert.doesNotMatch(
      source,
      /\b(export\s+)?const\s+(CLAIMS|EVIDENCE|PRODUCTS|RELEASES)\b/,
    );
  }
});
...
test("Evidence Peek preserves public DOM and truth state", () => {
  for (const snippet of [
    ...
    assert.ok(
      peek.includes(snippet),
      `missing public disclosure contract: ${snippet}`,
    );
  }
  assert.doesNotMatch(
    peek,
    /trust-score|confidence-score|verified-badge|certified/i,
  );
});
