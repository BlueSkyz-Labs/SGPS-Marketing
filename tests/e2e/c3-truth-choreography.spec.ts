...
test.describe("C3-B truth choreography", () => {
  ...
  test("state patterns preserve textual meaning without scoring", async ({ page }) => {
    ...
    for (const [state, style] of [
      ...
      expect(await item.evaluate((el) => getComputedStyle(el).borderTopStyle)).toBe(style);
    }
    expect(await page.locator("[data-truth-choreography]").innerText()).not.toMatch(/score|certified|verified|%/i);
  });

  test("no JavaScript and reduced motion retain state meaning", async ({ browser, page }) => {
    const context = await browser.newContext({ javaScriptEnabled: false, reducedMotion: "reduce" });
    try {
      const noJs = await context.newPage();
      await noJs.goto(`${origin}/truth-choreography/`);
      await expect(noJs.locator("[data-truth-state]")).toHaveCount(5);
      await expect(noJs.locator('[data-truth-state="not-published"]')).toHaveAccessibleName("Not published");
      const result = await noJs.locator('[data-truth-state="changed"]').evaluate((el) => ({
